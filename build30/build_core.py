...
ESCENARIOS = [
    ...
    ("Escalacion_Tarifa", "Esc_EscTarifa", "Escalación anual de la tarifa evitable", "%/año", FMT_PCT, {"X": 0.02 if V31 else 0.0, "C": 0.0, "B": 0.0, "F": 0.02}, False,
     "Base y Conservador: PLANA 0 % (P6, ácido). Favorable: +2 %/año (déficit tarifario 2026 = USD 621,9 M, Res. 029/25 → presión al alza). Custom v3.1: +2 %/año (fijado por Jorge, 04-sep-2026). Sensibilidad: tornado 10 §B (+1 pp sobre el Custom; r3)."),
...
def build_opex(wb):
    ...
    _inp = {r[1]: r[3] for r in INPUTS if r[1]}
    _fee, _seg, _pot = _inp["Fee_OM_kWp"], _inp["Seguro_kWp"], _inp["Potencia_DC"]
    _terr = _inp["Predial_Terreno"] if _inp["Comprador_Terreno"] == "SALELGI" else _inp["Renta_Terreno_ha"] * _pot / 1000 / _inp["Densidad_MWp_ha"]
    _opex_kwp = _fee + _seg + (_terr + _inp["Tributos_Locales"]) / _pot
    _f = lambda x, d=1: f"{x:.{d}f}".rstrip("0").rstrip(".").replace(".", ",")
    cbox = ws.cell(row=O["comp0"], column=8, value=f"Referencia: estudio CAPEX/OPEX (jul-2026), 20 $/kWp-año «todo incluido» a suelo (desglose en 12 §D). SALELGI paga fee {_f(_fee)} + seguros {_f(_seg)} + arriendo o predial + tributos ≈ {_f(_opex_kwp)} $/kWp-año (año 1, {_f(_pot / 1000)} MWp); el IVA de estos servicios es crédito y no se modela.")
...
def build_flujo(wb):
    ...
    kp = [
        # r3 (R3-5): guardia contra la raíz espuria de IRR en el dominio 1+r < 0 (TIR ≤ −100 % → "n/a"); detectada en el cruce aleatorio motor↔LibreOffice (08-sep-2026)
        ("TIR", "%", FMT_PCT2, f'=IFERROR(IRR({rng(fcf)}),"n/a")', f'=IFERROR(IF(IRR({rng(eq)},0.02)<=-1,"n/a",IRR({rng(eq)},0.02)),"n/a")', "TIR_Proyecto", "TIR_Equity", "Apalancamiento positivo si la TIR del accionista supera la del proyecto. «n/a»: sin deuda o sin solución (TIR ≤ −100 %: el accionista cubre el servicio con flujos negativos casi todo el horizonte)."),
...
def build_motor(wb, cases):
    ...
    for j, c in enumerate(cases):
        ...
        # v3.1: semilla 2 % — con 100 % de deuda LibreOffice no converge desde la semilla por defecto (Excel: mismo valor)
        # r3 (R3-5): IRR ≤ −100 % es una raíz espuria (dominio 1+r < 0; el accionista cubre el servicio con flujos negativos casi todo el horizonte) → "n/a"
        ws[f"{X}{OUT_ROWS['TIR_eq']}"] = f'=IF({S["D"]}>0,IFERROR(IF(IRR({eq},0.02)<=-1,"n/a",IRR({eq},0.02)),"n/a"),"n/a")'
